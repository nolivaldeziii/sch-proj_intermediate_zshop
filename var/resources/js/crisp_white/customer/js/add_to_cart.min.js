(function(){function flyToCart(element){var target=$('.lc-minicart:visible'),item=getProductRepresentationFor(element);if(target.length&&item.element&&item.element.length)$(item.element).fly(target,{view:item.view})};decorate('ProductsListView','addToCart',function(element){flyToCart(element);return arguments.callee.previousMethod.apply(this,arguments)});core.bind('popup.open',function(){$('body > img.photo').remove()})
